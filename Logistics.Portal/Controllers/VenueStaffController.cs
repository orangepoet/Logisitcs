...
namespace Logistics.Portal.Controllers {
    public class VenueStaffController : BaseController {
        [Inject]
        public IVenueStaffRep Service { get; set; }
        ...
        public JsonResult GetGrid() {
            ...
            IEnumerable<VenueStaff> source = null;
            if (PG.asc) {
                source = list.OrderBy(GetOrderBy(PG.sort));
            } else {
                source = list.OrderByDescending(GetOrderBy(PG.sort));
            }
            return Json(new { Rows = PagedList<VenueStaff>(source), Total = total });
        }
        ...
        [HttpPost]
        public JsonResult Create(VenueStaff model) {
            try {
        ...
        public JsonResult Delete() {
            try {
                ...
                List<VenueStaff> models = JsonConvert.DeserializeObject<List<VenueStaff>>(Request["data"]);
        ...
        private Func<VenueStaff, object> GetOrderBy(string sort) {
            return v => {
                switch (sort) {
                    default:
                        return v.Id;
